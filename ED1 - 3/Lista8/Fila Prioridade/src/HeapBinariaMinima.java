public class HeapBinariaMinima
{
	...
	public HeapBinariaMinima(HeapBinariaMaxima heapMaxima) {
		int tamanho = heapMaxima.getTamanho();
		int[] elementos = new int[tamanho];
		
		// Extrai os elementos da heap máxima em ordem reversa
		for (int i = tamanho; i > 0; i--) {
			elementos[tamanho - i] = heapMaxima.removeMax();
		}
		
		// Cria a heap mínima com os elementos extraídos
		tam = tamanho;
		vetor = new int[tamanho + 1];
		n = tamanho;
		
		for (int i = 0; i < tamanho; i++) {
			vetor[i + 1] = elementos[i];
		}
