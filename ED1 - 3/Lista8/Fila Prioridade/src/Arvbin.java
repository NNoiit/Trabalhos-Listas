...
public class Arvbin<T extends Comparable<T>>
{
	...
	public List<T> percorrerArvore(Arvbin<T> arvore) {
    List<T> elementos = new ArrayList<>();
    percorrerArvoreAux(arvore, elementos);
    return elementos;
}

	private void percorrerArvoreAux(Arvbin<T> no, List<T> elementos) {
		if (no == null) {
			return;
		}
		
		// Visita o nó antes de percorrer as subárvores
		elementos.add(no.retornaVal());
		
		// Percorre a subárvore esquerda
		percorrerArvoreAux(no.retornaEsq(), elementos);
		
		// Percorre a subárvore direita
		percorrerArvoreAux(no.retornaDir(), elementos);
	}
